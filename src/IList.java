...
interface IList<Cell> extends Iterable<Cell> {
	...
	boolean contains(Cell t);

	// filter out all items that don't pass the pred
	IList<Cell> filter(IPred<Cell> p);
...
class MtList implements IList<Cell> {
  ...
  // flood the cells that should be flooded in this list 
...
class ConsList implements IList<Cell> {
	...
	public IList<Cell> filter(IPred<Cell> p) {
	    if(p.apply(this.first)) {
	        return new ConsList(this.first, this.rest.filter(p));
	      }
	      else {
  ...
  // flood cells that should be flooded 
  public IList<Cell> flood() {
    // TODO Auto-generated method stub
    return null;
  }
